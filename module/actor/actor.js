...
export class PhoenixActor extends Actor {
    ...
    let d = new Dialog({
      ...
      buttons: {
        roll: {
          ...
          label: game.i18n.localize('Phoenix.Roll'),
  ...
  rollStat(attribute) {
    ...
    let d = new Dialog({
      title: "Select Roll Type",
      content: "<h2> "+game.i18n.localize('Phoenix.RollAdvantageDisadvantage')+ "</h2> <select style='margin-bottom:10px;'name='advantage' id='advantage'> <option value='none'>"+game.i18n.localize('Phoenix.RollNone')+"</option> <option value='advantage'>"+game.i18n.localize('Phoenix.RollAdvantageDisadvantage')+"</option></select> <br/>",
      buttons: {
        roll: {
          icon: '<i class="fas fa-check"></i>',
          label: game.i18n.localize('Phoenix.Roll'),
  ...
  rollItem(itemId, options = { event: null }) {
    ...
    if(item.type == "weapon"){
      ...
      let t = new Dialog({
        title: game.i18n.localize('Phoenix.RollSelectStat'),
        content: "<h2> "+game.i18n.localize('Phoenix.RollEnhanced')+"/"+game.i18n.localize('Phoenix.RollImpaired')+" </h2> <select style='margin-bottom:10px;'name='enhanced' id='enhanced'>\
        <option value='normal'>"+game.i18n.localize('Phoenix.RollNormal')+"</option>\
        <option value='enhanced'>"+game.i18n.localize('Phoenix.RollEnhanced')+"</option>\
        <option value='impaired'>"+game.i18n.localize('Phoenix.RollImpaired')+"</option></select> <br/>",
        buttons: {
          roll: {
            icon: '<i class="fas fa-check"></i>',
            label: game.i18n.localize('Phoenix.Roll'),
      ...
      let t = new Dialog({
        title: "Select Stat",
        content: "<h2> "+game.i18n.localize('Phoenix.RollPowerDesc')+" </h2> <input style='margin-bottom:10px;' name='power' id='power' value='1'></input><br/>",
        buttons: {
          roll: {
            icon: '<i class="fas fa-check"></i>',
            label: game.i18n.localize('Phoenix.Roll'),
  ...
  // this.rollWeapon(item, item.system.weapon.dmg2);
    ...
    var templateData = {
      ...
      item: item,
      pip: pipHtml,
      rollTitle: game.i18n.localize('Phoenix.RollDamage'), //The title of the roll.
    ...
    let template = 'systems/phoenix/templates/chat/statroll.html';
  ...
  rollSpell(item = "", power = ""){
    ...
    item.system.description += "<h2>"+game.i18n.localize('Phoenix.RollUsage')+": <strong>"+usage+"</strong></h2>";
    if(miscast){
      let miscastDesc = game.i18n.localize('Phoenix.RollMiscastDesc');
      miscastDesc = miscastDesc.replace("!miscast!", ""+miscast);
      item.system.description += "<h2>"+game.i18n.localize('Phoenix.RollMiscast')+": <strong>"+miscast+"</strong> </h2>" + miscastDesc;
    }
    ...
    var templateData = {
      ...
      isSpell: true,
      isWeapon:true,
      rollTitle: game.i18n.localize('Phoenix.RollSum')+"|"+game.i18n.localize('Phoenix.RollDice'), //The title of the roll.
  ...
  rollAttribute(attribute, advantage, item = "", rollOver = false) {
    ...
    if (rollOver == true) {
        resultText = (r._total >= targetValue ? game.i18n.localize('Phoenix.RollSuccess') : game.i18n.localize('Phoenix.RollFailure'));
    } else {
        resultText = (r._total <= targetValue ? game.i18n.localize('Phoenix.RollSuccess') : game.i18n.localize('Phoenix.RollFailure'));
    }
    ...
    var templateData = {
      actor: this,
      stat: {
        name: game.i18n.localize('Phoenix.'+attributeName).toUpperCase()
      },
      data: {
        diceTotal: {
          value: r._total,
